...
router.get('/', authObj.isLoggedIn, async (req, res, next) => {
    // for displaying posts with the hiddenHashtag of @Instagram
    const posts = await Post.find({ hiddenHashtags: { $in: ['#@Instagram'] } }).populate('comments').sort({ createdAt: 'desc' });
    const postsNum = await Post.find({ hiddenHashtags: { $in: ['#@Instagram'] } }).countDocuments();

    res.render('instagramPosts.ejs', { name: req.user.username, post: new Post(), posts: posts, postsNum: postsNum, csrfToken: req.csrfToken(), message: req.flash('message') })
});
...
router.post('/', authObj.isAuthenticated, async (req, res, next) => {
    ...
    req.post = new Post();
    next()
}, savePostAndRedirect('/instagramPosts'));
...
function savePostAndRedirect(path) {
  return async (req, res) => {
    ...
    try {
      if (post.source.substring(0,25) == "https://www.instagram.com") {
